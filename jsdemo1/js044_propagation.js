document.querySelector(".red").onclick = (e) => {
  alert("red");
  e.stopPropagation();
}; //querySelector: 요소 선택(단일, 다중, 이벤트 위임, 스타일 조작)

document.querySelector(".green").onclick = (e) => {
  alert("green");
  e.stopPropagation();
}; //클래스가 .green인 요소 클릭했을 때 실행.
...
document.querySelector("a").onclick = (e) => {
  ...
  //e.preventDefault();
  return false;
}; //문서 내의 첫 번째 <a>요소를 클릭했을 때 실행. 클릭하면 '안녕하세요' 알림표시. e.preventDefault()나 return false를 통해 이벤트 전파 중지.
